...
class ANS_WP_Import extends WP_Importer {
    ...
    function import_start($file) {
        if (!is_file($file)) {
            echo '<p><strong>' . esc_html__('Sorry, there has been an error.', 'ansar-import') . '</strong><br />';
            echo esc_html__('The file does not exist, please try again.', 'ansar-import') . '</p>';
            $this->footer();
            die();
        }
    ...
    function import_end() {
        ...
        echo '<p>' . esc_html__('All done.', 'ansar-import') . ' <a href="' . esc_url(admin_url()) . '">' . esc_html__('Have fun!', 'ansar-import') . '</a>' . '</p>';
        echo '<p>' . esc_html__('Remember to update the passwords and roles of imported users.', 'ansar-import') . '</p>';
...
//			echo '<p><strong>' . esc_html__( 'Sorry, there has been an error.', 'ansar-import' ) . '</strong><br />';
//			printf( esc_html__( 'The export file could not be found at <code>%s</code>. It is likely that this was caused by a permissions problem.', 'ansar-import' ), esc_html( $file['file'] ) );
//			echo '</p>';
//			return false;
//		}
    ...
    function import_options() {
        ...
        <form action="<?php echo esc_url(admin_url('admin.php?import=wordpress&amp;step=2')); ?>" method="post">
            ...
            <?php if ($this->allow_fetch_attachments()) : ?>
                <h3><?php esc_html_e('Import Attachments', 'ansar-import'); ?></h3>
                <p>
                    <input type="checkbox" value="1" name="fetch_attachments" id="import-attachments" />
                    <label for="import-attachments"><?php esc_html_e('Download and import file attachments', 'ansar-import'); ?></label>
                </p>
            <?php endif; ?>

            <p class="submit"><input type="submit" class="button" value="<?php esc_attr_e('Submit', 'ansar-import'); ?>" /></p>
        </form>
        <?php
    }
    ...
    function author_select($n, $author) {
        esc_html_e('Import author:', 'ansar-import');
        ...
        if ($create_users) {
            ...
            if ($this->version != '1.0') {
                esc_html_e('or create new user with login name:', 'ansar-import');
                $value = '';
            } else {
                esc_html_e('as a new user:', 'ansar-import');
        ...
        if (!$create_users && $this->version == '1.0') {
            esc_html_e('assign posts to an existing user:', 'ansar-import');
        } else {
            esc_html_e('or assign posts to an existing user:', 'ansar-import');
        }
        echo '</label>';
        
        $n_sanitized = absint($n);
    ...
    function process_menu_item($item) {
        ...
        if (!$menu_slug) {
            esc_html_e('Menu item skipped due to missing menu slug', 'ansar-import');
            echo '<br />';
            return;
        }
    ...
    function process_attachment($post, $url) {
        if (!$this->fetch_attachments)
            return new WP_Error('attachment_processing_error', __('Fetching attachments is not enabled', 'ansar-import'));
        ...
        if ($info = wp_check_filetype($upload['file']))
            ...
            return new WP_Error('attachment_processing_error', __('Invalid file type', 'ansar-import'));
    ...
    function fetch_remote_file($url, $post) {
        ...
        if (!$tmp_file_name) {
            return new WP_Error('import_no_file', __('Could not create temporary file.', 'ansar-import'));
        }
        ...
        if (is_wp_error($remote_response)) {
            ...
            return new WP_Error(
                    'import_file_error', sprintf(
                            ...
                            __('Request failed due to an error: %1$s (%2$s)', 'ansar-import'), esc_html($remote_response->get_error_message()), esc_html($remote_response->get_error_code())
                    )
            );
        }
        ...
        if (200 !== $remote_response_code) {
            ...
            return new WP_Error(
                    'import_file_error', sprintf(
                            ...
                            __('Remote server returned the following unexpected result: %1$s (%2$s)', 'ansar-import'), get_status_header_desc($remote_response_code), esc_html($remote_response_code)
                    )
            );
        }
        ...
        if (!$headers) {
            @unlink($tmp_file_name);
            return new WP_Error('import_file_error', __('Remote server did not respond', 'ansar-import'));
        }
        ...
        if (0 === $filesize) {
            @unlink($tmp_file_name);
            return new WP_Error('import_file_error', __('Zero size file downloaded', 'ansar-import'));
        }

        if (!isset($headers['content-encoding']) && isset($headers['content-length']) && $filesize !== (int) $headers['content-length']) {
            @unlink($tmp_file_name);
            return new WP_Error('import_file_error', __('Downloaded file has incorrect size', 'ansar-import'));
        }
        ...
        if ((!$type || !$ext ) && !current_user_can('unfiltered_upload')) {
            return new WP_Error('import_file_error', __('Sorry, this file type is not permitted for security reasons.', 'ansar-import'));
        }
        ...
        if (!$move_new_file) {
            @unlink($tmp_file_name);
            return new WP_Error('import_file_error', __('The uploaded file could not be moved', 'ansar-import'));
        }
    ...
    function header() {
        echo '<div class="wrap">';
        echo '<h2>' . esc_html__('Import WordPress', 'ansar-import') . '</h2>';
    ...
    function greet() {
        ...
        echo '<p>' . esc_html__('Howdy! Upload your WordPress eXtended RSS (WXR) file and we&#8217;ll import the posts, pages, comments, custom fields, categories, and tags into this site.', 'ansar-import') . '</p>';
        echo '<p>' . esc_html__('Choose a WXR (.xml) file to upload, then click Upload file and import.', 'ansar-import') . '</p>';
