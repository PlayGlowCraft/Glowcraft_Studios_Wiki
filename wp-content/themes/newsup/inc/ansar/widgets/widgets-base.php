    ...
    public function newsup_sanitize_data($instance, $new_instance) {
        if (is_array($this->text_fields)) {
            ...
            foreach ($this->text_fields as $field) {
                $instance = array_merge($instance, $this->newsup_update_text($field, $new_instance));
            }
        }
        ...
        if (is_array($this->url_fields)) {
            ...
            foreach ($this->url_fields as $field) {
                $instance = array_merge($instance, $this->newsup_update_url($field, $new_instance));
            }
        }
        ...
        if (is_array($this->text_areas)) {
            ...
            foreach ($this->text_areas as $field) {
                $instance = array_merge($instance, $this->newsup_update_textarea($field, $new_instance));
            }
        }
        if (is_array($this->checkboxes)) {
            // update the checkbox fields values
            foreach ($this->checkboxes as $field) {
                $instance = array_merge($instance, $this->newsup_update_checkbox($field, $new_instance));
            }
        }
        if (is_array($this->select_fields)) {
            // update the select fields values
            foreach ($this->select_fields as $field) {
                $instance = array_merge($instance, $this->newsup_update_select($field, $new_instance));
            }
        }
        return $instance;
    }
