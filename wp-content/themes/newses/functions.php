<?php define( 'NEWSES_THEME_DIR', get_template_directory() . '/' );
	...
	require_once get_template_directory() . '/inc/ansar/customizer-admin/newses-admin-plugin-install.php';
...
function newses_widgets_init() {
	...
	register_sidebar( array(
		...
		'description'   => '',
		'before_widget' => '<div class="col-md-'.$newses_footer_column_layout.'"><div id="%1$s" class="mg-widget %2$s">',
...
function newses_add_home_link($items, $args){
    if( $args->theme_location == 'primary' ){
        $item = '<li class="active home"><a class="nav-link homebtn" title="Home" href="'. esc_url( home_url() ) .'">' . "<span class='fa-solid fa-house-chimney'></span>" . '</a></li>';
        $items = $item . $items;
    }
    return $items;
}
